...
export default function BrandsPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (modalMode === 'create') {
        ...
        add('Brand created successfully', 'success');
      } else {
        await api.patch(`/api/brands?id=${brandId}`, payload);
        add('Brand updated successfully', 'success');
      }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filter Bar */}
      <div className="rounded-xl border bg-white p-4 flex flex-col sm:flex-row gap-4 items-center justify-between shadow-sm">
        <div className="flex gap-4 w-full sm:w-auto">
            <select 
                className="rounded-md border px-3 py-2 bg-gray-50 outline-none focus:ring-2 focus:ring-blue-500 w-full sm:w-auto" 
                value={activeFilter} 
                  ...
                  <tr key={b.id} className="hover:bg-blue-50 transition group">
                    <td className="px-4 py-2">
                        <div className="h-10 w-10 rounded border bg-gray-50 flex items-center justify-center overflow-hidden">
                            {b.image ? (
                                <img src={`${ASSET_BASE}/${b.image}`} alt="" className="w-full h-full object-cover" onError={(e) => e.target.style.display='none'} />
                            ) : (
                    ...
                    <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${b.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                            {b.is_active ? 'Active' : 'Inactive'}
                        </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2 opacity-100 transition-opacity">
                        <button onClick={() => openEdit(b)} className="p-1.5 rounded hover:bg-gray-200 text-gray-600" title="Edit">
                           <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                        </button>
                        <button onClick={() => deleteBrand(b.id)} className="p-1.5 rounded hover:bg-red-100 text-red-500" title="Delete">
          ...
          <div className="w-full max-w-lg bg-white rounded-xl shadow-2xl p-6 transform transition-all">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
               ...
               <div className="border-t pt-4 mt-2">
                  <label className="text-xs font-semibold text-gray-500 uppercase block mb-2">Brand Logo</label>
